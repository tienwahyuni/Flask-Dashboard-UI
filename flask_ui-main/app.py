...
def index():
    ...
    appfam = family.pivot_table(index='App',values='Reviews',aggfunc='sum').sort_values(by = 'Reviews',ascending=False).head()
    appfam = appfam.reset_index()
    appfam['App Name'] = ['Mahjong','Toy Pop Cubes','Coloring & Learn','FR Calculator','Poop FR']
    appfam = appfam.set_index('App Name')
    appfam.plot.bar()
    plt.savefig('fam_bar.png',bbox_inches="tight") 
